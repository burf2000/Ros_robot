...
def cmd_vel_callback(cmd_vel):
    ...
    if (left_speed_out != 0):
        left_speed_out = left_speed_out / 5.5

    if (right_speed_out != 0):
        right_speed_out = right_speed_out / 5.5
...
def motor_control(left_speed_out, right_speed_out):
    ...
    sendString = "%d %d %d" % (int(right_speed_out * PWRDIV),left_speed_out * PWRDIV, counter)
